...
def fallback_agent_guide(payload: dict) -> dict:
    ...
    if stage == "start":
        return {
            ...
            "options": [
                f"{topic}进入一个具体使用现场，使用者、受影响者和不确定边界同时变得可见",
                f"{topic}背后的研究或设计条件进入公共流程，并开始影响不同角色的判断",
                f"{topic}的默认设想被一个反例打断，原本稳定的使用关系开始摇晃",
                f"如果{topic}成为未来现场里的默认条件，谁获得便利、谁承担风险",
            ],
    ...
    if stage == "tools":
        return {
            "question": "先选择一个工具介入当前情境线；选完后可以继续选工具，也可以进入情境生成前追问。",
            "options": ["警示故事", "未来锥", "未来三角", "影响轮", "体验阶梯"],
            "hint": "工具结果会成为冲突、角色、场景或逻辑节点。",
        }
    if stage == "scenario_probe":
        return {
            "question": f"这条线路要落到哪个具体现场里？先选一个角色、空间或临界动作。",
            "options": [
                f"{topic}进入真实使用现场时，最先被改变的是一线使用者的判断动作",
                "把镜头放到一个被系统影响、但很少发声的人身上",
                "从一次临时中止、拒绝或人工复核的瞬间开始",
                "让工具结果变成现场里可见的规则、标记或物件",
            ],
            "hint": "这一步决定最终情境的主场景，不会生成多个情境选项。",
        }
    if stage == "scenario_refine":
        return {
            "question": "工具结果里哪一个冲突、证据或视觉细节必须保留到最终情境中？",
            "options": [
                "保留一个具体角色的拒绝权和解释权",
                "保留工具揭示出的第一层连锁影响",
                "保留一个让系统暂时失效的边界条件",
    ...
    if stage == "scenario_ready":
        return {
            "question": "情境生成材料已经稳定。现在生成一个图文情境，或回到工具栏继续补工具结果。",
            "options": ["生成一个图文情境", "继续选择工具"],
            "hint": "最终只生成一个情境，并保留这几轮追问记录。",
        }
    return {
        "question": "下一步先补哪一块材料？",
...
def build_agent_guide_prompt(payload: dict) -> str:
    ...
    return (
        ...
        "当用户选择工具后，先推动两轮情境生成前追问：一轮锁定具体现场/角色/临界动作，一轮锁定工具结果中必须保留的冲突、证据或视觉细节；最终只生成一个情境，不生成三种情境选项。\n"
        "你只能生成当前步骤的下一轮引导问题和快捷填写选项，不要改变步骤，不要要求用户填写隐藏表单。\n"
        "所有 options 都会被用户点选后在发送时作为本轮材料确认；普通阶段的选项必须是可直接写入记录的短回答，不要写成“补充研究背景”“换个角度追问”这类命令按钮。\n"
